...
## Diagnostics: Gambletron Data Issues

```{r diagnostics}
#| echo: false
#| results: 'asis'

cat("\n\n### Gambletron Scraping Diagnostics\n\n")
cat("**Purpose:** These diagnostics help identify issues with gambletron data when rendering fails on GitHub Actions.\n\n")

if (exists("gambletron_diagnostics") && length(gambletron_diagnostics) > 0) {
  
  cat("#### Key Information\n\n")
  cat("- **Today's Date String (m/d format):** ", gambletron_diagnostics$today_date_string, "\n")
  cat("- **Skins Date:** ", gambletron_diagnostics$skins_date, "\n")
  cat("- **Live Games Count:** ", gambletron_diagnostics$live_games_count, "\n")
  cat("- **Starting Later Count:** ", gambletron_diagnostics$starting_later_count, "\n")
  cat("- **Finished Games Count:** ", gambletron_diagnostics$finished_games_count, "\n")
  cat("- **NBA Today Count (after filtering):** ", gambletron_diagnostics$nba_today_count, "\n\n")
  
  if (!is.null(gambletron_diagnostics$live_scrape_error)) {
    cat("**LIVE SCRAPE ERROR:** ", gambletron_diagnostics$live_scrape_error, "\n\n")
  }
  
  if (!is.null(gambletron_diagnostics$inactive_scrape_error)) {
    cat("**INACTIVE SCRAPE ERROR:** ", gambletron_diagnostics$inactive_scrape_error, "\n\n")
  }
  
  cat("#### Raw Gambletron Data (Before Filtering)\n\n")
  
  if (!is.null(gambletron_diagnostics$live_games_raw) && nrow(gambletron_diagnostics$live_games_raw) > 0) {
    cat("**Live Games (Raw):**\n\n")
    print(knitr::kable(gambletron_diagnostics$live_games_raw, format = "html") %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed")))
    cat("\n\n")
  }
  
  if (!is.null(gambletron_diagnostics$starting_later_raw) && nrow(gambletron_diagnostics$starting_later_raw) > 0) {
    cat("**Starting Later Games (Raw):**\n\n")
    print(knitr::kable(gambletron_diagnostics$starting_later_raw, format = "html") %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed")))
    cat("\n\n")
  }
  
  if (!is.null(gambletron_diagnostics$finished_games_raw) && nrow(gambletron_diagnostics$finished_games_raw) > 0) {
    cat("**Finished Games (Raw):**\n\n")
    print(knitr::kable(gambletron_diagnostics$finished_games_raw, format = "html") %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed")))
    cat("\n\n")
  }
  
  cat("#### Filtered NBA Data (After Date Filtering)\n\n")
  
  if (!is.null(gambletron_diagnostics$nba_today_data) && nrow(gambletron_diagnostics$nba_today_data) > 0) {
    cat("**NBA Today (After Filtering):**\n")
    cat("These games passed the filter: Live OR (Starting Later AND date matches today) OR (Finished AND date matches today)\n\n")
    print(knitr::kable(gambletron_diagnostics$nba_today_data, format = "html") %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed")))
    cat("\n\n")
  }
  
  cat("#### Suspicious Finished Games\n\n")
  
  if (!is.null(gambletron_diagnostics$suspicious_finished_games) && nrow(gambletron_diagnostics$suspicious_finished_games) > 0) {
    cat("**WARNING: These finished games have 100%/0% probabilities but are listed for today:**\n")
    cat("These are likely from previous days and should have been filtered out.\n\n")
    print(knitr::kable(gambletron_diagnostics$suspicious_finished_games, format = "html") %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed")))
    cat("\n\n")
  } else {
    cat("✓ No suspicious finished games detected\n\n")
  }
  
  cat("#### Final Live Data\n\n")
  
  if (!is.null(gambletron_diagnostics$live_data_final) && nrow(gambletron_diagnostics$live_data_final) > 0) {
    cat("**Live Data Joined to Today's Picks:**\n\n")
    print(knitr::kable(gambletron_diagnostics$live_data_final, format = "html") %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed")))
    cat("\n\n")
  }
  ...
  cat("#### Analysis\n\n")
  cat("**Common Issues:**\n\n")
  cat("1. **Finished games from previous days**: If 'Finished Games Count' > 0 and these games have today's date, they're incorrectly matched\n")
  cat("2. **100%/0% probabilities on upcoming games**: Indicates finished game data is being applied to wrong teams\n")
  cat("3. **Date format mismatch**: Check that 'Today's Date String' matches gambletron's date format\n")
  cat("4. **Timezone issues**: Verify that skins_date aligns with when gambletron updates its data\n\n")
  
} else {
  cat("No gambletron diagnostics available.\n")
}
```
