...
nba_today <- all_games %>%
  filter(
    sport == "NBA",
    (status == "Live" | game_date == today_date | status == "Finished")
  )
...
if (nrow(nba_today) > 0 && nrow(todays_picks) > 0) {
  ...
  live_data_team1 <- nba_today %>%
    select(Team = team_1, live_win_prob = team_1_prob, gambletron_status = status)
  
  live_data_team2 <- nba_today %>%
    select(Team = team_2, live_win_prob = team_2_prob, gambletron_status = status)
    
  live_data_final <- bind_rows(live_data_team1, live_data_team2) %>%
    mutate(live_win_prob = convert_prob_to_num(live_win_prob)) %>%
    # FIXED: Only replace "Blazers" when it's standalone
    mutate(
      Team = case_when(
        Team == "Blazers" ~ "Trail Blazers",
        TRUE ~ Team
      )
    ) %>%
  ...
  # If scrape fails or no games, create empty columns
  todays_picks <- todays_picks %>%
    mutate(Live_Expected_Skins = NA_real_, gambletron_status = NA_character_)
}
```
...
if (nrow(todays_picks) > 0) {
  ...
  parse_game_box <- function(box) {
    ...
    winner_abbr <- NA_character_
    if (is_final && !is.na(team1_score) && !is.na(team2_score)) {
      winner_abbr <- if_else(team1_score > team2_score, team1_name, team2_name)
    }
  ...
  if (nrow(matchup_game_results) > 0) {
    matchup_game_results <- matchup_game_results %>%
      mutate(
        ...
        Winner = str_replace_all(Winner, team_replacements)
      )
  }
  ...
  matchup_status_lookup <- data.frame(Abbr = character(), GameStatus = character(), Winner = character(), stringsAsFactors = FALSE)
  ...
  if (nrow(matchup_game_results) > 0) {
    ...
    team_1_status <- all_games_status %>%
      select(Abbr = Team1, GameStatus, Winner)
    
    team_2_status <- all_games_status %>%
      select(Abbr = Team2, GameStatus, Winner)
  ...
  # FIXED: Only override live probabilities for truly final games
  matchup_prep <- todays_picks %>%
    ...
    mutate(
      ...
      # FIXED: Only override if GameStatus is exactly "Final" AND Winner is determined
      Live_Expected_Skins_Corrected = case_when(
        GameStatus == "Final" & !is.na(Winner) & Abbr == Winner & Pick == "W" ~ 1.0,
        GameStatus == "Final" & !is.na(Winner) & Abbr == Winner & Pick == "L" ~ 0.0,
        GameStatus == "Final" & !is.na(Winner) & Abbr != Winner & Pick == "W" ~ 0.0,
        GameStatus == "Final" & !is.na(Winner) & Abbr != Winner & Pick == "L" ~ 1.0,
  ...
  matchup_table <- matchup_prep %>%
    ...
    summarise(
      ...
      Live_Skin_Prob_1 = first(Live_Expected_Skins_Corrected),
      ...
      Live_Skin_Prob_2 = if(n() > 1) nth(Live_Expected_Skins_Corrected, 2) else NA,
...
if (nrow(todays_picks) > 0) {
  ...
  # Use the corrected Live_Expected_Skins from matchup_prep if available
  if (exists("matchup_prep")) {
    todays_results <- matchup_prep %>%
      select(Date, Player, Team, Abbr, Pick, `Expected Skins`, 
             Won_Game, Game_Status, Live_Expected_Skins = Live_Expected_Skins_Corrected) %>%
      mutate(
    ...
    todays_results <- todays_picks %>%
      ...
      mutate(
        ...
        Skins_Luck = Actual_Skins - `Expected Skins`,
        Live_Expected_Skins = if_else("Live_Expected_Skins" %in% names(todays_picks), 
                                       todays_picks$Live_Expected_Skins, NA_real_)
      )
  }
...
# RECALCULATE BASELINE FROM CSV FILES - FIXED VERSION
historical_file <- file.path(history_dir, "ytd_baseline.csv")
baseline_date_file <- file.path(history_dir, "baseline_date.txt")

# Get all CSV files
all_csv_files <- list.files(history_dir, pattern = "^\\d{4}-\\d{2}-\\d{2}\\.csv$", full.names = TRUE)

if (length(all_csv_files) > 0) {
  # Read all data through Nov 1
  all_historical_data <- bind_rows(lapply(all_csv_files, function(f) {
    data <- read_csv(f, show_col_types = FALSE)
    data$source_file <- basename(f)
    return(data)
  }))
  
  # Filter for games through Nov 1
  baseline_cutoff <- as_date("2025-11-01")
  
  # FIXED: Count all games for expected skins, but only completed games for actual skins
  ytd_baseline <- all_historical_data %>%
    filter(Date <= baseline_cutoff) %>%
    group_by(Player) %>%
    summarise(
      Games = n(),  # Count ALL games
      Total_Expected_Skins = sum(Expected_Skins, na.rm = TRUE),  # Sum all expected
      Total_Actual_Skins = sum(Actual_Skins, na.rm = TRUE),  # Sum only completed (NAs become 0)
      Cumulative_Skins_Luck = sum(Skins_Luck, na.rm = TRUE),  # Sum only completed
      .groups = 'drop'
    ) %>%
    arrange(Player)
  
  # Save the recalculated baseline
  write_csv(ytd_baseline, historical_file)
  
  # Set baseline date to Nov 2 (so only Nov 2+ data gets added)
  if (!file.exists(baseline_date_file)) {
    writeLines("2025-11-02", baseline_date_file)
  }
  baseline_date <- as_date("2025-11-02")
  
} else {
  # No CSV files found, use empty baseline
...
if (length(all_files) > 0) {
  ...
  if (length(valid_files) > 0) {
    ...
    current_year <- year(today_ct)
    # FIXED: Include all games from valid files, not just completed ones
...
if (nrow(ytd_data) > 0) {
  ytd_new_data <- ytd_data %>%
    group_by(Player) %>%
    summarise(
      Games_new = n(),  # All games
      Total_Expected_Skins_new = sum(Expected_Skins, na.rm = TRUE),  # All expected
      Total_Actual_Skins_new = sum(Actual_Skins, na.rm = TRUE),  # Only completed
      Cumulative_Skins_Luck_new = sum(Skins_Luck, na.rm = TRUE),  # Only completed
      .groups = 'drop'
    )
