...
use_csv_for_sportsline <- current_hour_ct >= 12 | current_hour_ct < 2

cat("===== BLACKOUT WINDOW CHECK =====\n")
cat("In Blackout Window?:", use_csv_for_sportsline, "\n")
cat("Blackout window is: noon to 1:59 AM (01:59)\n")
...
cat("\n===== PROPOSED SOLUTION =====\n")
cat("During blackout window (noon - 2 AM):\n")
cat("1. Load CSV for expected skins data ✓ (already implemented)\n")
cat("2. Also load GAME RESULTS from the CSV ✗ (NOT implemented)\n")
cat("3. DO NOT scrape plaintextsports.com during blackout ✗ (currently scrapes anyway)\n")
cat("4. DO NOT scrape gambletron during blackout ✗ (currently scrapes anyway)\n")
...
if (use_csv_for_sportsline && file.exists(file_path)) {
  ...
  if ("game_id" %in% names(csv_data)) {
    ...
    cat("⚠️ CSV DOES NOT have game_id column!\n")
...
if (use_csv_for_sportsline) {
  ...
  cat("❌ **BROKEN BEHAVIORS:**\n\n")
  cat("1. `expected_skins_data` chunk: Loads CSV for skins_date (", as.character(skins_date), ") ✓\n")
  cat("2. `scrape_live_data` chunk: STILL scrapes gambletron for calendar date (", as.character(Sys.Date()), ") ❌\n")
  cat("3. `matchup_table` chunk: STILL scrapes plaintextsports for calendar date (", as.character(Sys.Date()), ") ❌\n")
...
cat("- Run between noon and 2 AM to see blackout issues\n")
cat("- Run between 2 AM - noon to see normal operation\n")
