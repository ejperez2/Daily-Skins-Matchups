...
# Updated baseline data as of today
ytd_baseline <- tribble(
  ~Player, ~Games, ~Total_Expected_Skins, ~Total_Actual_Skins, ~Cumulative_Skins_Luck,
  "Adam", 28, 15.7, 17, 1.3,
  "Eristeo", 26, 17.7, 20, 2.3,
  "Kenneth", 28, 18.6, 13, -5.6,
  "Brian", 26, 17.8, 18, 0.2,
  "Thomas", 26, 17.0, 12, -5.0,
  "Matt", 26, 17.1, 19, 1.9
)
...
# This preserves the original baseline date across all runs
baseline_date_file <- file.path(history_dir, "baseline_date.txt")
if (!file.exists(baseline_date_file)) {
  # Only set baseline on first run (or if file is missing)
