...
if (nrow(nba_today) > 0 && nrow(todays_picks) > 0) {
  ...
  # Fix: For teams in the same game with opposite picks, ensure equal Live Expected Skins
  # When one team has W pick with prob p and the other has L pick with prob (1-p),
  # both should have the same expected skins. Use the higher value to correct scraping errors.
  todays_picks <- todays_picks %>%
    group_by(game_id) %>%
    mutate(
      # Check if we have 2 teams with opposite picks and both have live data
      has_opposite_picks = n() == 2 & n_distinct(Pick) == 2 & sum(!is.na(Live_Expected_Skins)) == 2,
      # Use the maximum value for both teams (assumes the higher is more accurate)
      corrected_live = if_else(has_opposite_picks, 
                                max(Live_Expected_Skins, na.rm = TRUE), 
                                Live_Expected_Skins),
      Live_Expected_Skins = corrected_live
    ) %>%
    ungroup() %>%
    select(-has_opposite_picks, -corrected_live)
}
