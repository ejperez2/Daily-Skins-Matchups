...
if (nrow(nba_today) > 0 && nrow(todays_picks) > 0) {
  ...
  live_data_team1 <- nba_today %>%
    select(Team = team_1, live_win_prob = team_1_prob, status)
  
  live_data_team2 <- nba_today %>%
    select(Team = team_2, live_win_prob = team_2_prob, status)
    
  live_data_final <- bind_rows(live_data_team1, live_data_team2) %>%
    mutate(live_win_prob = convert_prob_to_num(live_win_prob)) %>%
    # FIXED: Only replace "Blazers" when it's not already "Trail Blazers"
    mutate(
      Team = case_when(
  ...
  todays_picks <- todays_picks %>%
    mutate(Live_Expected_Skins = NA_real_, status = NA_character_)
}
```
...
if (nrow(todays_picks) > 0) {
  ...
  # *** MODIFICATION: Joined with live data and add override logic for final games ***
  matchup_prep <- todays_picks %>%
    ...
    mutate(
      ...
      # OVERRIDE LOGIC: If game is Final in PlaintextSports, force live prob to 100%/0%
...
# RECALCULATE BASELINE FROM CSV FILES
...
if (length(all_csv_files) > 0) {
  # Read all data and filter for completed games through Nov 1
  ...
  # Filter for games through Nov 1 with actual results
  baseline_cutoff <- as_date("2025-11-01")
  ytd_baseline <- all_historical_data %>%
    filter(Date <= baseline_cutoff, !is.na(Actual_Skins)) %>%
    group_by(Player) %>%
    summarise(
      Games = n(),
      Total_Expected_Skins = sum(Expected_Skins, na.rm = TRUE),
