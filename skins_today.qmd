...
if (nrow(todays_picks) > 0) {
  ...
  player_summary <- todays_picks %>%
    ...
    mutate(
      ...
      `Expected Skins %` = `Total Expected Skins` / `Total Games`
    ) %>%
    select(Player, `Total Games`, `Total Expected Skins`, `Expected Skins %`, `Max Skins`) %>%
...
if (nrow(todays_picks) > 0) {
  ...
  url_scores <- paste0("https://cleaningtheglass.com/stats/games/?date=", today_ct)
