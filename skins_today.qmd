---
title: "Today's Skins"
# VERSION: 2024-11-20-v2 (Show game times in CT for upcoming games; blank scores for upcoming games)
format:
  html:
    self-contained: true
    theme: darkly
...
if (nrow(todays_picks) > 0) {
  ...
  # 7. Create score lookup for finished games
  finished_score_lookup <- data.frame(Abbr = character(), Final_Score = character(), stringsAsFactors = FALSE)
  
  if (nrow(all_game_results) > 0) {
    finished_only <- all_game_results %>% filter(Finished == TRUE & !is.na(Score1) & !is.na(Score2))
    if (nrow(finished_only) > 0) {
      team1_scores <- finished_only %>%
        mutate(Final_Score = paste0(Score1, "-", Score2)) %>%
        select(Abbr = Team1, Final_Score)
      team2_scores <- finished_only %>%
        mutate(Final_Score = paste0(Score2, "-", Score1)) %>%
        select(Abbr = Team2, Final_Score)
      finished_score_lookup <- bind_rows(team1_scores, team2_scores)
    }
  }
  
  todays_results <- todays_results %>%
    left_join(finished_score_lookup, by = "Abbr") %>%
    mutate(Final_Score = if_else(is.na(Final_Score), "", Final_Score))
  
  # 8. Save today's results to file (use underscores for easier CSV handling)
...
if (exists("todays_results") && nrow(todays_results) > 0) {
  ...
  total_row <- data.frame(
    ...
    `Total Games` = nrow(todays_results_with_cumulative),
  ...
  knitr::kable(
    ...
    column_spec(c(2, 3, 8, 9), bold = TRUE) %>%
...
if (nrow(todays_picks) > 0) {
  ...
  matchup_table <- matchup_prep %>%
    ...
    mutate(
      ...
      Combined_Score = if_else(
        str_detect(Status_CT, "\\d{1,2}:\\d{2}\\s*[AP]M\\s+CT"),  # If upcoming game with time
        "",  # Blank score
        if_else(is.na(Score_1) | Score_1 == "", "", Score_1)  # Otherwise use existing logic
      ),
      # Create vs. display
...
if (nrow(todays_picks) > 0) {
  ...
  if (exists("todays_results")) {
    team_table_data <- team_table_data %>%
      ...
      mutate(
        ...
        # Apply color-coding to Team column
        Team_Display = case_when(
          !is.na(Won_Skin) & Won_Skin == TRUE ~ 
            cell_spec(Team, background = "#90EE90", color = "#000000 !important", 
                      bold = TRUE, format = "html", extra_css = "color: #000000 !important;"),
          !is.na(Won_Skin) & Won_Skin == FALSE ~ 
            cell_spec(Team, background = "#FFB6C6", color = "#000000 !important", 
                      bold = TRUE, format = "html", extra_css = "color: #000000 !important;"),
          TRUE ~ Team
        )
      ) %>%
    ...
    # No results yet, use original Player and Team columns
    team_table_data <- team_table_data %>%
      mutate(Player_Display = Player, Team_Display = Team)
  }
  ...
  team_table_data <- team_table_data %>%
    ...
    select(Team = Team_Display, Pick, `Expected Skins`, Player = Player_Display, Moneyline, `Implied Prob`, Player_Raw = Player) %>%
...
if (file.exists(yesterday_file)) {
  ...
  if (nrow(yesterday_data) > 0) {
    ...
    # Parse game boxes to get actual matchups and scores
    parse_game_box <- function(box) {
      parts <- strsplit(box, "\\|")[[1]]
      status <- str_trim(parts[2])
      team1_line <- str_trim(parts[4])
    ...
    if (length(yesterday_boxes) > 0) {
      ...
      # Create score lookup for finished games
      yesterday_score_lookup <- data.frame(Abbr = character(), Scraped_Score = character(), stringsAsFactors = FALSE)
      finished_yesterday <- yesterday_matchups %>% filter(Finished == TRUE & !is.na(Score1) & !is.na(Score2))
      if (nrow(finished_yesterday) > 0) {
        score_team1 <- finished_yesterday %>%
          mutate(Scraped_Score = paste0(Score1, "-", Score2)) %>%
          select(Abbr = Team1, Scraped_Score)
        score_team2 <- finished_yesterday %>%
          mutate(Scraped_Score = paste0(Score2, "-", Score1)) %>%
          select(Abbr = Team2, Scraped_Score)
        yesterday_score_lookup <- bind_rows(score_team1, score_team2)
      }
      ...
      # Safely add Final_Score column if it doesn't exist in the CSV
      if (!"Final_Score" %in% names(yesterday_data)) {
        yesterday_data <- yesterday_data %>% mutate(Final_Score = NA_character_)
      }
      
      yesterday_with_game <- yesterday_data %>%
        left_join(team_to_game, by = "Abbr") %>%
        left_join(yesterday_score_lookup, by = "Abbr") %>%
        mutate(
          # Use scraped score if available, otherwise fall back to stored Final_Score column
          Final_Score_Display = case_when(
            !is.na(Scraped_Score) & Scraped_Score != "" ~ Scraped_Score,
            !is.na(Final_Score) & Final_Score != "" ~ as.character(Final_Score),
            TRUE ~ ""
          ),
          Won_Skin = case_when(
            is.na(Won_Game) ~ NA,
      ...
      yesterday_matchup_table <- yesterday_with_game %>%
        ...
        summarise(
          ...
          Pick_1 = first(Pick),
          Won_Skin_1 = first(Won_Skin),
          Expected_Skin_1 = round(first(Expected_Skins), 1),
          Player_2 = if(n() > 1) nth(Player, 2) else NA_character_,
          Team_2 = if(n() > 1) nth(Team, 2) else NA_character_,
          Pick_2 = if(n() > 1) nth(Pick, 2) else NA_character_,
          Won_Skin_2 = if(n() > 1) nth(Won_Skin, 2) else NA,
          Expected_Skin_2 = if(n() > 1) round(nth(Expected_Skins, 2), 1) else NA_real_,
          Final_Score_Game = first(Final_Score_Display),
          Status = first(Game_Status),
          .groups = 'drop'
        ) %>%
        mutate(
        ...
        select(
          ...
          Pick = Pick_1,
          `Won Skin` = Won_1,
          `Expected Skin` = Expected_Skin_1,
          VS,
          `Player 2` = Player_2_Display,
          `Team 2` = Team_2_Display,
          `Pick 2` = Pick_2_Display,
          `Won Skin 2` = Won_2,
          `Expected Skin 2` = Expected_Skin_2,
          `Final Score` = Final_Score_Game
        )
      ...
      if (nrow(yesterday_matchup_table) > 0) {
        ...
        yesterday_player_summary <- yesterday_with_game %>%
          ...
          mutate(
            `Luck Leverage` = if_else(
              Total_Expected_Skins == 0,
              "0%",
              paste0(sprintf("%.0f", (Skins_Luck_Total / Total_Expected_Skins) * 100), "%")
            )
          ) %>%
          rename(
          ...
          mutate(
            # Apply color coding to Player column - FIXED: exclude values between -0.1 and 0.1
            Player_Display = case_when(
              `Skins Luck` > 0.1 ~ 
                cell_spec(Player, background = "#90EE90", color = "#000000 !important", 
                          bold = TRUE, format = "html", extra_css = "color: #000000 !important;"),
        ...
        total_row <- data.frame(
          ...
          `Luck Leverage` = if_else(
            sum(yesterday_with_game$Expected_Skins, na.rm = TRUE) == 0,
            "0%",
            paste0(sprintf("%.0f", (sum(yesterday_with_game$Skins_Luck, na.rm = TRUE) / 
             sum(yesterday_with_game$Expected_Skins, na.rm = TRUE)) * 100), "%")
          ),
          check.names = FALSE,
          stringsAsFactors = FALSE
        )
...
```{r yesterday_diagnostics}
#| echo: false
#| message: false
#| warning: false
#| results: 'asis'

cat("\n\n### Yesterday's Results Diagnostics\n\n")

cat("- **Yesterday date:** ", as.character(yesterday_ct), "\n")
cat("- **Yesterday file path:** ", file.path(history_dir, paste0(yesterday_ct, ".csv")), "\n")
cat("- **Yesterday file exists:** ", file.exists(file.path(history_dir, paste0(yesterday_ct, ".csv"))), "\n")

if (file.exists(file.path(history_dir, paste0(yesterday_ct, ".csv")))) {
  ydiag <- read_csv(file.path(history_dir, paste0(yesterday_ct, ".csv")), show_col_types = FALSE)
  cat("- **Rows in yesterday CSV:** ", nrow(ydiag), "\n")
  cat("- **Columns in yesterday CSV:** ", paste(names(ydiag), collapse = ", "), "\n")
  cat("- **Has Final_Score column:** ", "Final_Score" %in% names(ydiag), "\n")
  cat("- **Has Scraped_Score column (from join):** Only present after join with scrape lookup\n")
  
  # Try scraping yesterday's plaintextsports page
  tryCatch({
    ydiag_url <- paste0("https://plaintextsports.com/nba/", yesterday_ct, "/")
    ydiag_page <- read_html(ydiag_url)
    ydiag_text <- ydiag_page %>% html_element("body") %>% html_text2()
    ydiag_lines <- strsplit(ydiag_text, "\n")[[1]]
    ydiag_boxes <- ydiag_lines[grepl("^\\+", ydiag_lines)]
    cat("- **Game boxes scraped from plaintextsports:** ", length(ydiag_boxes), "\n")
    if (length(ydiag_boxes) > 0) {
      cat("- **First box (raw):** `", ydiag_boxes[1], "`\n")
    }
  }, error = function(e) {
    cat("- **Scrape error:** ", as.character(e), "\n")
  })
} else {
  cat("- **ERROR:** Yesterday's CSV not found — this is why the table is missing.\n")
}
```
...
if (length(all_files) > 0) {
  ...
  if (length(valid_files) > 0) {
    ...
    ytd_data <- all_history %>%
      filter(Date >= baseline_date, !is.na(Actual_Skins))
...
if (nrow(ytd_data) > 0) {
  ...
  ytd_summary_with_total <- ytd_summary_with_total %>%
    ...
    select(Player, `Cumulative Skins Luck` = `Cumulative Skins Luck Display`, Games, 
           `Total Expected Skins`, `Total Actual Skins`, `Expected Skins %`)
...
cat("\n\n# 🛠️ Deep Diagnostics: Missing Table & YTD Check\n\n")

cat("### 1. Date Calculations\n\n")
cat("- **Current System Time:** ", as.character(Sys.time()), "\n")
cat("- **Calculated 'Today' (Skins Date):** ", as.character(today_ct), "\n")
cat("- **Calculated 'Yesterday':** ", as.character(yesterday_ct), "\n")

cat("\n### 2. File Check for Yesterday\n\n")
target_file <- file.path(history_dir, paste0(yesterday_ct, ".csv"))
cat("- **Looking for file:** ", target_file, "\n")
...
if(file_exists) {
  ...
  cat("**First 5 rows of yesterday's data:**\n")
  print(knitr::kable(head(df, 5), format = "html") %>% kable_styling(bootstrap_options = "condensed"))
} else {
  cat("**ERROR:** Yesterday's CSV file was not found. This is why the table is missing.\n")
}

cat("\n### 3. YTD Inclusion Check\n\n")
cat("- **Baseline Date:** ", as.character(baseline_date), "\n")
cat("- **Condition for inclusion:** File Date >= Baseline Date\n")
cat("- **Is Yesterday >= Baseline?** ", yesterday_ct >= baseline_date, "\n")

# List files that qualify
all_files <- list.files(history_dir, pattern = "^\\d{4}-\\d{2}-\\d{2}\\.csv$", full.names = TRUE)
valid_files <- all_files[sapply(all_files, function(f) {
  file_date <- as_date(str_extract(basename(f), "\\d{4}-\\d{2}-\\d{2}"))
  return(!is.na(file_date) && file_date >= baseline_date)
...
cat("- **Total files included in YTD:** ", length(valid_files), "\n")
cat("- **Is yesterday's file in this list?** ", target_file %in% valid_files, "\n")

cat("\n### 4. Scraping Simulation (Yesterday)\n\n")
cat("Attempting to scrape yesterday's results page to see if matchups can be found.\n")
test_url <- paste0("[https://plaintextsports.com/nba/](https://plaintextsports.com/nba/)", yesterday_ct, "/")
cat("- **URL:** ", test_url, "\n")

tryCatch({
  page_test <- read_html(test_url)
  text_test <- page_test %>% html_element("body") %>% html_text2()
  lines_test <- strsplit(text_test, "\n")[[1]]
  boxes_test <- lines_test[grepl("^\\+", lines_test)]
  cat("- **Game Boxes Found:** ", length(boxes_test), "\n")
  
  if(length(boxes_test) == 0) {
    cat("**CRITICAL ISSUE:** The scraper found 0 games for yesterday. This causes the join to fail, resulting in an empty table even if the CSV exists.\n")
  } else {
    cat("**Scrape Successful.** The issue might be in matching team abbreviations.\n")
  }
}, error = function(e) {
  cat("**SCRAPE ERROR:** ", as.character(e), "\n")
})
