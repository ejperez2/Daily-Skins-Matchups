  ...
  if (nrow(yesterday_data) > 0) {
    ...
    if (length(yesterday_boxes) > 0) {
      ...
      yesterday_with_game <- yesterday_data %>%
        ...
        mutate(
          # Use scraped score if available, otherwise fall back to stored Final_Score column (if it exists)
          Final_Score_Display = case_when(
            !is.na(Scraped_Score) & Scraped_Score != "" ~ Scraped_Score,
            "Final_Score" %in% names(yesterday_data) & !is.na(Final_Score) & Final_Score != "" ~ as.character(Final_Score),
            TRUE ~ ""
          ),
          Won_Skin = case_when(
            is.na(Won_Game) ~ NA,
...
if (nrow(ytd_data) > 0) {
  ...
  ytd_summary <- ytd_baseline_renamed %>%
    ...
    mutate(
      ...
      Actual_Skins_Pct = if_else(Games == 0, 0, (Total_Actual_Skins / Games) * 100)
    ) %>%
    rename(
  ...
  weighted_avg_actual_pct <- if_else(total_games == 0, 0, (total_actual / total_games) * 100)
  ...
  total_row <- data.frame(
    ...
    `Actual Skins %` = weighted_avg_actual_pct,
    check.names = FALSE
  )
  ...
  ytd_summary_with_total <- ytd_summary_with_total %>%
    ...
    select(Player, `Cumulative Skins Luck` = `Cumulative Skins Luck Display`, Games, 
           `Total Expected Skins`, `Total Actual Skins`, `Expected Skins %`, `Actual Skins %`)
