...
# Initialize diagnostic data frames
gambletron_raw_scrape <- data.frame()
gambletron_processing_log <- data.frame()
...
# Store raw scraped data for diagnostics BEFORE any processing
gambletron_raw_scrape <- all_games
...
if (nrow(nba_today) > 0 && nrow(todays_picks) > 0) {
  ...
  convert_prob_to_num <- function(prob_str) {
    if (is.na(prob_str) || prob_str == "") return(NA_real_)
    num_val <- as.numeric(str_remove(prob_str, "%")) / 100.0
    # Validate probability is in reasonable range
    if (is.na(num_val) || num_val < 0 || num_val > 1) return(NA_real_)
    return(num_val)
  }
  ...
  # Store processing log for diagnostics
  gambletron_processing_log <- nba_today_normalized %>%
    select(game_pair_id, team_1, team_2, team_1_prob, team_2_prob, 
           prob_1_num, prob_2_num, prob_1_normalized, prob_2_normalized, status)
  
  # Create one row per team with standardized names
  live_data_team1 <- nba_today_normalized %>%
    select(Team = team_1, live_win_prob = prob_1_normalized, game_pair_id, status)
  
  live_data_team2 <- nba_today_normalized %>%
    select(Team = team_2, live_win_prob = prob_2_normalized, game_pair_id, status)
  ...
  # Prioritize: Live > Starting Later > Finished
  ...
  todays_picks <- todays_picks %>%
    ...
    mutate(
      Live_Expected_Skins = case_when(
        is.na(live_win_prob) ~ `Expected Skins`,  # CHANGED: Default to Expected Skins if no live data
...
if (exists("todays_results") && nrow(todays_results) > 0) {
  ...
  # Add TOTAL row - FIXED: Use NA_real_ for Perfect Skins % to match numeric type
  total_row <- data.frame(
    ...
    `Perfect Skins %` = NA_real_,  # FIXED: Use NA_real_ to match numeric type
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
...
if (exists("gambletron_raw_scrape") && nrow(gambletron_raw_scrape) > 0) {
  cat("**Raw scraped data from Gambletron (before any processing):**\n\n")
  
  diagnostic_raw <- gambletron_raw_scrape %>%
    filter(!is.na(sport), sport == "NBA") %>%
    select(status, team_1, team_2, team_1_prob, team_2_prob, game_date) %>%
    head(15)
  
  if (nrow(diagnostic_raw) > 0) {
    print(knitr::kable(diagnostic_raw, format = "html") %>%
      kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE))
  } else {
    cat("No NBA games found in raw scrape.\n")
  }
  
  cat("\n\n")
} else {
  cat("No raw scrape data available.\n\n")
}

if (exists("gambletron_processing_log") && nrow(gambletron_processing_log) > 0) {
  cat("**Processing log (probability conversion and normalization):**\n\n")
  
  processing_summary <- gambletron_processing_log %>%
    select(game_pair_id, team_1, team_2, 
           raw_prob_1 = team_1_prob, raw_prob_2 = team_2_prob,
           num_prob_1 = prob_1_num, num_prob_2 = prob_2_num,
           final_prob_1 = prob_1_normalized, final_prob_2 = prob_2_normalized,
           status) %>%
    head(15)
  
  print(knitr::kable(processing_summary, digits = 3, format = "html") %>%
    kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE, font_size = 11))
  
  cat("\n\n")
}

cat("**Current picks with Expected vs Live Expected Skins:**\n\n")

if (exists("todays_picks") && nrow(todays_picks) > 0) {
  picks_diagnostic <- todays_picks %>%
    select(Player, Team, Pick, `Expected Skins`, Live_Expected_Skins, Abbr) %>%
    mutate(
      Difference = Live_Expected_Skins - `Expected Skins`,
      Has_Live_Data = abs(Difference) > 0.01
    ) %>%
    arrange(Player, desc(abs(Difference)))
  
  print(knitr::kable(picks_diagnostic, digits = 3, format = "html") %>%
    kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE))
  
  cat("\n\n**Key Observations:**\n")
  cat("- Teams with 'Has_Live_Data = TRUE' successfully got live probabilities from Gambletron\n")
  cat("- Teams with 'Has_Live_Data = FALSE' are using Expected Skins as fallback\n")
  cat("- Large differences (>0.2) may indicate odds have moved significantly\n\n")
  
  # Specific check for Matt and Kenneth
  cat("**Specific checks for Matt and Kenneth (Thunder, Magic):**\n\n")
  
  matt_kenneth_teams <- picks_diagnostic %>%
    filter(Player %in% c("Matt", "Kenneth"), Team %in% c("Oklahoma City Thunder", "Orlando Magic"))
  
  if (nrow(matt_kenneth_teams) > 0) {
    print(knitr::kable(matt_kenneth_teams, digits = 3, format = "html") %>%
      kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE))
    
    cat("\n")
    
    # Check what was scraped for these teams
    if (exists("gambletron_processing_log") && nrow(gambletron_processing_log) > 0) {
      thunder_magic_scrape <- gambletron_processing_log %>%
        filter(str_detect(team_1, regex("Thunder|Magic", ignore_case = TRUE)) | 
               str_detect(team_2, regex("Thunder|Magic", ignore_case = TRUE)))
      
      if (nrow(thunder_magic_scrape) > 0) {
        cat("\n**What Gambletron returned for Thunder/Magic games:**\n\n")
        print(knitr::kable(thunder_magic_scrape, digits = 3, format = "html") %>%
          kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE))
      } else {
        cat("\nNo Thunder or Magic games found in Gambletron scrape - this is the problem!\n")
      }
    }
  } else {
    cat("Matt and Kenneth's Thunder/Magic picks not found in today's picks.\n")
  }
