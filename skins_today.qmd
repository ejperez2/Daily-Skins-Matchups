...
if (nrow(todays_picks) > 0) {
  ...
  # FIX: Correctly formatted as a plain string
    ...
    all_games_status <- matchup_game_results %>%
      mutate(
        GameStatus = case_when(
          Finished == TRUE ~ "Final",
          # FIX: Show specific progress, strip prefixes
  ...
  matchup_prep <- todays_picks %>%
    ...
    mutate(
      ...
      # Use the actual game status from scraping, default to "Not Started" if no match
...
# This correctly uses today_ct (which is skins_date) to find yesterday
...
if (file.exists(yesterday_file)) {
  ...
  if (nrow(yesterday_data) > 0) {
    ...
    # Check if CSV has complete Won_Game data
    has_incomplete_data <- any(is.na(yesterday_data$Won_Game))
    
    if (has_incomplete_data) {
      # CSV was saved before all games finished - scrape to get complete results
      # FIX: Correctly formatted as a plain string
      url_scores_yesterday <- "https://plaintextsports.com/nba/"
      page_scores_yesterday <- read_html(url_scores_yesterday)
      
      # Get all text
      all_text_yesterday <- page_scores_yesterday %>%
        html_element("body") %>%
        html_text2()
      
      # Extract score boxes
      all_lines_yesterday <- strsplit(all_text_yesterday, "\n")[[1]]
      score_boxes_yesterday <- all_lines_yesterday[grepl("^\\+", all_lines_yesterday)]
      ...
      # Parse all games
      if (length(score_boxes_yesterday) > 0) {
        yesterday_game_results <- bind_rows(lapply(score_boxes_yesterday, parse_game_box))
      } else {
      ...
      if (nrow(yesterday_game_results) > 0) {
        yesterday_game_results <- yesterday_game_results %>%
          mutate(
            Team1 = str_replace_all(Team1, team_replacements),
            Team2 = str_replace_all(Team2, team_replacements)
          )
      ...
      # Create Won_Game lookup from scraped data
      scraped_won_lookup <- data.frame(Abbr = character(), Won_Game_Scraped = logical(), stringsAsFactors = FALSE)
      
      if (nrow(yesterday_game_results) > 0) {
        finished_games <- yesterday_game_results %>% filter(Finished == TRUE)
        
        if (nrow(finished_games) > 0) {
          team_1 <- finished_games %>%
            select(Abbr = Team1, PTS = Score1, Opp_PTS = Score2) %>%
            mutate(Won_Game_Scraped = PTS > Opp_PTS)
          
          team_2 <- finished_games %>%
            select(Abbr = Team2, PTS = Score2, Opp_PTS = Score1) %>%
            mutate(Won_Game_Scraped = PTS > Opp_PTS)
          
          scraped_won_lookup <- bind_rows(team_1, team_2) %>%
            select(Abbr, Won_Game_Scraped)
        }
      }
      
      # Merge scraped data with CSV, preferring CSV but using scraped for NA values
      yesterday_with_game <- yesterday_data %>%
        left_join(scraped_won_lookup, by = "Abbr") %>%
        mutate(
          # Use CSV Won_Game if available, otherwise use scraped data
          Won_Game_Final = if_else(is.na(Won_Game), Won_Game_Scraped, Won_Game)
        )
    } else {
      # CSV has complete data - use it directly
      yesterday_with_game <- yesterday_data %>%
        mutate(Won_Game_Final = Won_Game)
    }
    
    # Create game_id and calculate Won_Skin
    yesterday_with_game <- yesterday_with_game %>%
      arrange(Player, Team) %>%
      mutate(
        game_id = paste0("game_", ceiling(row_number() / 2)),
        Won_Skin = case_when(
          is.na(Won_Game_Final) ~ NA,
          Pick == "W" & Won_Game_Final == TRUE ~ TRUE,
          Pick == "L" & Won_Game_Final == FALSE ~ TRUE,
          TRUE ~ FALSE
        ),
        Game_Status = ifelse(is.na(Won_Game_Final), "Not Started", "Final")
      )
    ...
    if (nrow(yesterday_matchup_table) > 0) {
      ...
      # Add player summary for yesterday
...
# FIX: Unconditionally write today's date to the baseline_date file.
# This uses today_ct (which is skins_date)
...
# FIX: This single chunk now prepares the data AND generates the cumulative table
# to prevent "object not found" errors.

# --- Part 1: Data Preparation ---
ytd_data <- data.frame() # Initialize an empty dataframe
all_files <- list.files(history_dir, pattern = "^\\d{4}-\\d{2}-\\d{2}\\.csv$", full.names = TRUE)

if (length(all_files) > 0) {
  # Filter files to include only dates on or after the baseline_date
  ...
  if (length(valid_files) > 0) {
    # Read and combine all new daily data
    all_history <- bind_rows(lapply(valid_files, function(f) read_csv(f, show_col_types = FALSE)))
    current_year <- year(today_ct)
    # Filter for the current year and for completed games only
    ytd_data <- all_history %>%
      filter(year(Date) == current_year, !is.na(Actual_Skins))
  }
}

# --- Part 2: Generate and Print Table ---
if (nrow(ytd_data) > 0) {
  # Calculate summary stats from the new data
  ...
  # FIX: Rename baseline columns *before* joining
  ...
  # Combine renamed baseline with new data
  ytd_summary <- ytd_baseline_renamed %>%
    full_join(ytd_new_data, by = "Player") %>%
    mutate(
      # FIX: This coalesce logic now works because the columns exist
  ...
  # No new data found, so just display the baseline table
...
# Only generate charts if we have NEW data from the previous chunk
if (exists("ytd_data") && nrow(ytd_data) > 0) {
  
  # FIX: Summarize the daily data to get ONE point per player per day.
  # This prevents multiple points for the current day.
  ...
  # Create the baseline starting point (the first data point)
  baseline_start <- ytd_baseline %>%
    mutate(
      Date = min(ytd_data$Date) - days(1), # Set baseline date to 1 day before first new data
  ...
  # Combine baseline start point with the new SUMMARIZED daily data
  chart_data <- daily_data_summarized %>%
    bind_rows(baseline_start) %>%
    # Arrange by date so the cumsum is in the correct order
    arrange(Player, Date) %>%
    group_by(Player) %>%
    mutate(
      # Calculate the true running total
  ...
  p1 <- ggplot(chart_data, aes(x = Date, y = Cumulative_Luck, color = Player, group = Player)) +
    ...
    theme(
      ...
      text = element_text(colour = "#FFFFFF"), # White text for dark mode
      axis.text = element_text(colour = "#FFFFFF"), # White axis text
      axis.title = element_text(colour = "#FFFFFF"), # White axis titles
      plot.background = element_rect(fill = "#222222", color = NA), # Dark plot bg
      panel.background = element_rect(fill = "#222222", color = NA), # Dark panel bg
      legend.background = element_rect(fill = "#222222", color = NA), # Dark legend bg
      legend.key = element_rect(fill = "#222222", color = NA), # Dark legend key
      panel.grid.major = element_line(color = "#444444"), # Darker grid lines
      panel.grid.minor = element_line(color = "#333333")  # Even darker grid lines
    ) +
