...
if (nrow(todays_picks) > 0) {
  ...
  # --- NEW URL FIX ---
  # Use live URL during the day, date-specific URL during blackout
  if (is_blackout_window) {
    # Blackout: use today's date-specific URL for FINAL scores
    url_scores <- paste0("https://plaintextsports.com/nba/", today_ct, "/")
  } else {
    # Not blackout: use the LIVE, generic URL
  ...
  if (nrow(matchup_game_results) > 0) {
    ...
    # (This includes the typo fix from last time)
  ...
  matchup_prep <- todays_picks %>%
    ...
    # (This includes the 0%/100% fix from last time)
    mutate(
      Live_Expected_Skins = case_when(
...
page_scores <- read_html(url_scores)
  ...
  completed_games_table <- todays_results %>%
    ...
    arrange(Player, Team) %>%
    # --- *** NEW FIX: Convert Won Game? to character *** ---
    mutate(`Won Game?` = as.character(`Won Game?`))
    # --- *** END NEW FIX *** ---
  
  if (nrow(completed_games_table) > 0) {
    
    # --- Create total row (with blank fix) ---
    total_row_completed <- data.frame(
      Player = "TOTAL",
      Team = "",  
      Pick = "",  
      `Won Game?` = "", # Now works because column is character
    ...
    knitr::kable(
      ...
      na = "" # Keep this for any other NAs
    ) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
      column_spec(c(5, 7), bold = TRUE) %>%
      row_spec(nrow(completed_games_table_with_total), bold = TRUE, background = "#444444") # Add bold row
