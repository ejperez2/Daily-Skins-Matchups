...
cat(sprintf(
  "<p style='font-size:14px; margin-top:0px; margin-bottom:16px;'>
...
# NEW APPROACH: Use CSV after 7 PM instead of complex blackout window
# After 7 PM, use saved CSV data for expected skins (sportsline data)
# Gambletron and plaintextsports will still be scraped for live updates
use_csv_for_sportsline <- current_hour_ct >= 19  # 7 PM or later
...
if (use_csv_for_sportsline) {
  # --- BRANCH 1: LOAD FROM CSV (After 7 PM) ---
  cat("NOTICE: After 7 PM - loading expected skins from today's CSV\n")
  ...
  if (file.exists(file_path)) {
    ...
    # Check if the loaded CSV has the 'game_id' column.
    if ("game_id" %in% colnames(csv_data)) {
      # --- CSV is NEW (has game_id) ---
      cat("Successfully loaded data from:", file_path, "(with game_id)\n")
      
      # *** THIS IS THE FIX ***
      # We just select the columns we need. We DO NOT create a new game_id.
      todays_picks <- csv_data %>%
        mutate(
          "Expected Skins" = Expected_Skins,
          Moneyline = NA_real_,
          "Implied Prob" = NA_real_
          # 'game_id' column is read directly from the CSV
        ) %>%
        select(Date, Team, Pick, "Expected Skins", Player, Moneyline, "Implied Prob", Abbr, game_id) %>%
        arrange(desc(`Expected Skins`)) # This sort is fine, it doesn't affect the game_id
        
    } else {
      # --- CSV is OLD (no game_id) ---
      # This is a fallback for a broken file.
      # We must re-scrape to get the correct game_ids.
      cat("WARNING: CSV file is old (no game_id). Re-scraping sportsline to get correct game matchups...\n")
      use_csv_for_sportsline <- FALSE
    }
    
  } else {
    # After 7 PM but no file found - try to scrape anyway
    cat("WARNING: After 7 PM but no CSV found. Attempting to scrape sportsline...\n")
    use_csv_for_sportsline <- FALSE
  }
}

if (!use_csv_for_sportsline) {
  # --- BRANCH 2: SCRAPE FROM WEB (Before 7 PM or if CSV missing) ---
  cat("NOTICE: Scraping data from sportsline.com\n")
  ...
  todays_picks <- game_data %>%
    ...
    mutate(
      ...
      prob_no_.vig = implied_prob / total_prob_in_game
    ) %>%
    ungroup() %>%
...
if (nrow(nba_today) > 0 && nrow(todays_picks) > 0) {
  ...
  live_data_final <- bind_rows(live_data_team1, live_data_team2) %>%
    ...
    # *** FIX APPLIED HERE ***
    # Changed all `==` to `str_detect` to flexibly match team names
    # from different scrapers (e.g., "Knicks" vs "NY Knicks")
    mutate(
      Team = case_when(
        str_detect(Team, "Blazers") ~ "Portland Trail Blazers",
        str_detect(Team, "76ers") ~ "Philadelphia 76ers",
        str_detect(Team, "Pistons") ~ "Detroit Pistons",
        str_detect(Team, "Heat") ~ "Miami Heat",
        str_detect(Team, "Celtics") ~ "Boston Celtics",
        str_detect(Team, "Nets") ~ "Brooklyn Nets",
        str_detect(Team, "Knicks") ~ "New York Knicks",
        str_detect(Team, "Raptors") ~ "Toronto Raptors",
        str_detect(Team, "Bulls") ~ "Chicago Bulls",
        str_detect(Team, "Cavaliers") ~ "Cleveland Cavaliers",
        str_detect(Team, "Pacers") ~ "Indiana Pacers",
        str_detect(Team, "Bucks") ~ "Milwaukee Bucks",
        str_detect(Team, "Hawks") ~ "Atlanta Hawks",
        str_detect(Team, "Hornets") ~ "Charlotte Hornets",
        str_detect(Team, "Magic") ~ "Orlando Magic",
        str_detect(Team, "Wizards") ~ "Washington Wizards",
        str_detect(Team, "Nuggets") ~ "Denver Nuggets",
        str_detect(Team, "Timberwolves") ~ "Minnesota Timberwolves",
        str_detect(Team, "Thunder") ~ "Oklahoma City Thunder",
        str_detect(Team, "Jazz") ~ "Utah Jazz",
        str_detect(Team, "Warriors") ~ "Golden State Warriors",
        str_detect(Team, "Clippers") ~ "Los Angeles Clippers",
        str_detect(Team, "Lakers") ~ "Los Angeles Lakers",
        str_detect(Team, "Suns") ~ "Phoenix Suns",
        str_detect(Team, "Kings") ~ "Sacramento Kings",
        str_detect(Team, "Mavericks") ~ "Dallas Mavericks",
        str_detect(Team, "Rockets") ~ "Houston Rockets",
        str_detect(Team, "Grizzlies") ~ "Memphis Grizzlies",
        str_detect(Team, "Pelicans") ~ "New Orleans Pelicans",
        str_detect(Team, "Spurs") ~ "San Antonio Spurs",
        TRUE ~ Team
      )
    ) %>%
...
```{r COMPREHENSIVE_DIAGNOSTIC}
#| echo: true
#| message: true
#| warning: true

# This chunk will diagnose the entire data pipeline for the matchup table.

# --- 0. Load Libraries ---
if (!require("pacman")) install.packages("pacman")
pacman::p_load(rvest, httr, dplyr, tidyr, stringr, lubridate, tibble, knitr, kableExtra, readr)

cat("--- STARTING COMPREHENSIVE DIAGNOSTIC ---\n\n")

# --- 1. Diagnose Time and Branching ---
cat("--- STEP 1: TIME & BRANCHING LOGIC ---\n")
current_time_ct <- now(tzone = "America/Chicago")
current_hour_ct <- hour(current_time_ct)
if (current_hour_ct < 2) {
  skins_date <- as_date(current_time_ct) - days(1)
...
use_csv_for_sportsline <- current_hour_ct >= 19

cat(paste("  Current CT Time:", current_time_ct, "\n"))
cat(paste("  Skins Date:", as.character(skins_date), "\n"))
cat(paste("  Is it after 7 PM? (use_csv_for_sportsline):", use_csv_for_sportsline, "\n"))
cat("  CONCLUSION: The script is correctly entering the 'load from CSV' branch.\n\n")

# --- 2. Diagnose CSV Loading ---
cat("--- STEP 2: CSV FILE CONTENTS ---\n")
history_dir <- "skins_history"
file_path <- file.path(history_dir, paste0(skins_date, ".csv"))

if (!file.exists(file_path)) {
  cat("  CRITICAL ERROR: CSV file not found at:", file_path, "\n")
} else {
  csv_data <- read_csv(file_path, show_col_types = FALSE)
  cat(paste("  Loaded:", basename(file_path), "(", nrow(csv_data), "rows)\n"))
  cat("  CSV Columns:", paste(colnames(csv_data), collapse = ", "), "\n")
  
  if (!"game_id" %in% colnames(csv_data)) {
    cat("  WARNING: The 'game_id' column is MISSING from the CSV file.\n")
  }
  
  cat("  First 6 rows from CSV (note the 'Expected_Skins' sorting):\n")
  print(head(csv_data %>% select(Player, Team, Expected_Skins)))
  cat("  CONCLUSION: The CSV is sorted by 'Expected_Skins', NOT by game.\n\n")
}

# --- 3. Diagnose 'todays_picks' Creation (THE BUG) ---
cat("--- STEP 3: 'todays_picks' DATA MANIPULATION (THE BUG) ---\n")
cat("  Replicating the *broken* logic from 'expected_skins_data' chunk:\n")

# This is the broken logic from that chunk
todays_picks_broken <- csv_data %>%
  mutate(
    "Expected Skins" = Expected_Skins,
    Moneyline = NA_real_,
    "Implied Prob" = NA_real_,
    # This line is the bug. It assumes CSV is ordered by game, but it's not.
    game_id = paste0(Date, "_", ceiling(row_number() / 2))
  ) %>%
  select(Date, Team, Pick, "Expected Skins", Player, Moneyline, "Implied Prob", Abbr, game_id) %>%
  arrange(game_id) # Sort by the new, broken game_id

cat("  Resulting 'todays_picks' data with BROKEN 'game_id's:\n")
print(todays_picks_broken %>% select(Player, Team, "Expected Skins", game_id))
cat("  CONCLUSION: The 'game_id' is being incorrectly assigned. For example, '..._1' is pairing Cleveland and Boston.\n\n")


# --- 4. Diagnose 'matchup_prep' Creation (THE SYMPTOM) ---
cat("--- STEP 4: HEAD-TO-HEAD TABLE DATA (THE SYMPTOM) ---\n")
cat("  Replicating the 'group_by' logic from 'matchup_table' chunk:\n")

matchup_table_broken_data <- todays_picks_broken %>%
  group_by(game_id) %>%
  summarise(
...
cat("  This is the final broken data used to build your table:\n")
print(matchup_table_broken_data)
cat("  CONCLUSION: This confirms the matchups are wrong (e.g., CLE vs. BOS).\n\n")


# --- 5. Diagnose Scrape (GROUND TRUTH) ---
cat("--- STEP 5: 'plaintextsports.com' SCRAPE (GROUND TRUTH) ---\n")
cat("  Scraping live data to see the *actual* matchups...\n")
...
parse_game_box_simple <- function(box) {
  ...
  team_replacements <- c("^GS$" = "GSW", "^NO$" = "NOP", "^NY$" = "NYK", "^SA$" = "SAS", "^WSH$" = "WAS", "^PHX$"="PHX", "^GSW$"="GSW", "^LAC$"="LAC", "^LAL$"="LAL", "^SAC$"="SAC", "^DAL$"="DAL", "^HOU$"="HOU", "^MEM$"="MEM", "^NOP$"="NOP", "^SAS$"="SAS", "^DEN$"="DEN", "^MIN$"="MIN", "^OKC$"="OKC", "^POR$"="POR", "^UTA$"="UTA", "^BOS$"="BOS", "^BKN$"="BKN", "^NYK$"="NYK", "^PHI$"="PHI", "^TOR$"="TOR", "^CHI$"="CHI", "^CLE$"="CLE", "^DET$"="DET", "^IND$"="IND", "^MIL$"="MIL", "^ATL$"="ATL", "^CHA$"="CHA", "^MIA$"="MIA", "^ORL$"="ORL", "^WAS$"="WAS")
  
  team1_name <- str_replace_all(team1_name, team_replacements)
  team2_name <- str_replace_all(team2_name, team_replacements)
  
  data.frame(Matchup = paste(team1_name, "vs.", team2_name), Status = status)
}

if (length(score_boxes) > 0) {
  actual_matchups <- bind_rows(lapply(score_boxes, parse_game_box_simple))
  cat("  Found", nrow(actual_matchups), "actual games from scrape:\n")
  print(actual_matchups)
  cat("  CONCLUSION: The scrape is working. The *actual* matchups (e.g., CLE vs. PHI) are present.\n\n")
} else {
  cat("  ERROR: Could not scrape plaintextsports.com.\n\n")
}

# --- 6. FINAL DIAGNOSIS & SOLUTION ---
cat("--- FINAL DIAGNOSIS ---\n")
cat("1.  The problem is NOT the scrape. The scrape is working.\n")
cat("2.  The problem is 100% in the 'expected_skins_data' chunk.\n")
cat("3.  After 7 PM, it loads the CSV. This CSV (2025-11-05.csv) is sorted by 'Expected_Skins' and, more importantly, **DOES NOT have a 'game_id' column**.\n")
cat("4.  The code then *invents* a new 'game_id' based on the row number (`ceiling(row_number() / 2)`). This incorrectly pairs teams (CLE+BOS, MIN+IND, etc.).\n")
cat("5.  This broken `game_id` is then used in the 'matchup_table' chunk, resulting in the incorrect table you see.\n\n")

cat("--- RECOMMENDED SOLUTION (2 Parts) ---\n")
cat("1.  FIX 'results_and_luck' chunk: We must modify it to **SAVE** the correct 'game_id' (from the daytime scrape) into the .csv file. This is the permanent fix for tomorrow.\n")
cat("2.  FIX 'expected_skins_data' chunk: We must modify it to **RE-SCRAPE** if it loads a CSV *without* a 'game_id' column (this will fix the table for *today*).\n")
...
if (nrow(todays_picks) > 0) {
  ...
  matchup_prep <- todays_picks %>%
    ...
    arrange(game_id) # This sort is now on the CORRECT game_id
  ...
  matchup_table <- matchup_prep %>%
    ...
    summarise(
      ...
      Live_Skin_Prob_1 = first(Live_Expected_Skins), 
      Player_2 = if(n() > 1) nth(Player, 2) else NA_character_,
      Team_2 = if(n() > 1) nth(Team, 2) else NA_character_,
      Pick_2 = if(n() > 1) nth(Pick, 2) else NA_character_,
      Won_Skin_2 = if(n() > 1) nth(Won_Skin, 2) else NA,
      Live_Skin_Prob_2 = if(n() > 1) nth(Live_Expected_Skins, 2) else NA,
...
if (nrow(todays_picks) > 0) {
  ...
  if (nrow(completed_games_table) > 0) {
    ...
    total_row <- data.frame(
      ...
      Pick = "",
      `Won Game?` = NA,
      `Actual Skins` = sum(completed_games_table$`Actual Skins`, na.rm = TRUE),
      `Expected Skins` = sum(completed_games_table$`Expected Skins`, na.rm = TRUE),
      `Skins Luck` = sum(completed_games_table$`Skins Luck`, na.rm = TRUE),
      check.names = FALSE,
      stringsAsFactors = FALSE
    )
    ...
    knitr::kable(completed_games_with_total, digits = 1, align = 'c', caption = "Skins Results for Completed Games") %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
...
if (exists("todays_results") && nrow(todays_results) > 0) {
  ...
  total_row <- data.frame(
    Player = "TOTAL",
    `Total Expected Skins` = sum(final_player_summary$`Total Expected Skins`, na.rm = TRUE),
    `Live Expected Skins` = sum(final_player_summary$`Live Expected Skins`, na.rm = TRUE),
    `Total Actual Skins` = sum(final_player_summary$`Total Actual Skins`, na.rm = TRUE),
    `Skins Luck` = sum(final_player_summary$`Skins Luck`, na.rm = TRUE),
    `Live Skins Luck` = sum(final_player_summary$`Live Skins Luck`, na.rm = TRUE),
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
  ...
  if (nrow(yesterday_data) > 0) {
    ...
    if (length(yesterday_boxes) > 0) {
      ...
      if (nrow(yesterday_matchup_table) > 0) {
        ...
        total_row <- data.frame(
          Player = "TOTAL",
          `Total Expected Skins` = sum(yesterday_player_summary$`Total Expected Skins`, na.rm = TRUE),
          `Total Actual Skins` = sum(yesterday_player_summary$`Total Actual Skins`, na.rm = TRUE),
          `Skins Luck` = sum(yesterday_player_summary$`Skins Luck`, na.rm = TRUE),
          check.names = FALSE,
          stringsAsFactors = FALSE
        )
