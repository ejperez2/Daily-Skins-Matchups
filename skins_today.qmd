...
if (nrow(nba_today) > 0 && nrow(todays_picks) > 0) {
  ...
  live_data_final <- live_data_standardized %>%
    ...
    ungroup() %>%
    select(Team, live_win_prob = normalized_prob, game_pair_id) %>%
    # Remove duplicates after standardization, keeping first occurrence
    group_by(Team) %>%
    slice(1) %>%
    ungroup() %>%
  ...
  todays_picks <- todays_picks %>%
    ...
    mutate(
      # Check if this player has both teams in this game with opposite picks
      player_has_both_sides = n() == 2 & n_distinct(Pick) == 2,
      both_have_live_data = sum(!is.na(Live_Expected_Skins)) == 2
    ) %>%
    ungroup() %>%
    group_by(game_id) %>%
    mutate(
      # For any player with both sides, use the average (which should be the same if probabilities are complementary)
      # If not the same, this averaging will ensure they're equal
      needs_correction = any(player_has_both_sides & both_have_live_data),
      avg_live_expected = if_else(needs_correction, mean(Live_Expected_Skins, na.rm = TRUE), NA_real_),
      Live_Expected_Skins = if_else(needs_correction & !is.na(avg_live_expected),
                                     avg_live_expected,
                                     Live_Expected_Skins)
    ) %>%
    ungroup() %>%
    select(-player_has_both_sides, -both_have_live_data, -needs_correction, -avg_live_expected)
}
...
if (exists("todays_picks") && nrow(todays_picks) > 0) {
  ...
  if (nrow(thomas_picks) > 0) {
    ...
    cat("- If they're different, there may be a team name matching issue or scraping problem.\n\n")
    ...
    thomas_same_game <- thomas_picks %>%
      ...
      summarise(
        ...
        live_skins = paste(round(Live_Expected_Skins, 3), collapse = " and "),
        are_equal = abs(diff(Live_Expected_Skins)) < 0.01,
        .groups = 'drop'
      )
