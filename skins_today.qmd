...
This section scrapes `plaintextsports.com/nba` for game results. 
...
# This overwrites any old data and ensures we start fresh
  ...
  if (length(valid_files) > 0) {
    ...
    if (nrow(ytd_data) > 0) {
      ...
      ytd_summary <- ytd_baseline %>%
        full_join(ytd_new_data, by = "Player", suffix = c("_baseline", "_new")) %>%
        mutate(
        ...
        mutate(Avg_Skins_Luck_per_Game = Cumulative_Skins_Luck / Games) %>%
        rename(
          `Total Expected Skins` = Total_Expected_Skins,
          `Total Actual Skins` = Total_Actual_Skins,
      ...
      # No new data yet, just show baseline
      ytd_summary <- ytd_baseline %>%
        mutate(Avg_Skins_Luck_per_Game = Cumulative_Skins_Luck / Games) %>%
        rename(
          `Total Expected Skins` = Total_Expected_Skins,
    ...
    # No valid files yet, just show baseline
    ytd_summary <- ytd_baseline %>%
      mutate(Avg_Skins_Luck_per_Game = Cumulative_Skins_Luck / Games) %>%
      rename(
        `Total Expected Skins` = Total_Expected_Skins,
    ...
    knitr::kable(
      ...
      digits = 1,
      align = 'c',
      caption = paste0(current_year, " Year-to-Date Cumulative Stats (Baseline)")
    ) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
      column_spec(5, bold = TRUE)
  }
} else {
  # No daily files yet, just show baseline
  ytd_summary <- ytd_baseline %>%
    mutate(Avg_Skins_Luck_per_Game = Cumulative_Skins_Luck / Games) %>%
    rename(
      `Total Expected Skins` = Total_Expected_Skins,
...
# Only generate charts if we have NEW data (from baseline date forward)
if (length(all_files) > 0) {
  # Filter files: only include dates >= baseline_date
  valid_files <- all_files[sapply(all_files, function(f) {
    file_date <- as_date(str_extract(basename(f), "\\d{4}-\\d{2}-\\d{2}"))
  ...
  if (length(valid_files) > 0) {
    ...
    if (nrow(ytd_data) > 0) {
      ...
      # Calculate cumulative stats by date for each player
      daily_cumulative <- ytd_data %>%
        arrange(Player, Date) %>%
        group_by(Player) %>%
        mutate(
          Cumulative_Expected = cumsum(Expected_Skins),
          Cumulative_Actual = cumsum(Actual_Skins),
          Cumulative_Luck = cumsum(Skins_Luck)
        ) %>%
        ungroup()
      
      # Add baseline as the starting point (one day before first NEW data)
      baseline_start <- ytd_baseline %>%
        mutate(
          Date = min(ytd_data$Date) - days(1),  # Day before first NEW recorded data
          Cumulative_Expected = Total_Expected_Skins,
          Cumulative_Actual = Total_Actual_Skins,
          Cumulative_Luck = Cumulative_Skins_Luck
        ) %>%
        select(Player, Date, Cumulative_Expected, Cumulative_Actual, Cumulative_Luck)
      
      # Combine baseline with daily data
      chart_data <- daily_cumulative %>%
        select(Player, Date, Cumulative_Expected, Cumulative_Actual, Cumulative_Luck) %>%
      ...
      p1 <- ggplot(chart_data, aes(x = Date, y = Cumulative_Luck, color = Player, group = Player)) +
        ...
        labs(
          title = "Cumulative Skins Luck Over Time",
          x = "Date",
          y = "Cumulative Skins Luck",
          color = "Player"
        ) +
        theme_minimal() +
        theme(
      ...
      p2 <- ggplot(chart_data, aes(x = Date, y = Cumulative_Expected, color = Player, group = Player)) +
        ...
        geom_point(size = 2) +
        labs(
          title = "Total Expected Skins Over Time",
          x = "Date",
          y = "Cumulative Expected Skins",
          color = "Player"
        ) +
        theme_minimal() +
        theme(
