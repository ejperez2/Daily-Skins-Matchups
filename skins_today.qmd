...
# *** FIX: Skip scraping during blackout window ***
if (!use_csv_for_sportsline) {
  # Only scrape if NOT in blackout window
...
if (nrow(todays_picks) > 0) {
  ...
  if (!use_csv_for_sportsline) {
    ...
    # During blackout, use data from CSV (Won_Game already loaded)
    todays_picks <- todays_picks %>%
      mutate(
        Won_Skin = case_when(
          Won_Game == TRUE & Pick == "W" ~ TRUE,
        ...
        Game_Status = if_else(is.na(Won_Game), "Not Started", "Final")
      )
  }
...
if (nrow(todays_picks) > 0) {
  ...
  # *** FIX: Skip scraping during blackout - results already in todays_picks from CSV ***
  if (!use_csv_for_sportsline) {
    # Only scrape if NOT in blackout window
    ...
    # During blackout, todays_picks already has Won_Game, Actual_Skins, Skins_Luck from CSV
    todays_results <- todays_picks
  }
...
if (exists("todays_results") && nrow(todays_results) > 0) {
  ...
  final_player_summary <- todays_results %>%
    ...
    summarise(
      ...
      `Live Expected Skins` = sum(Live_Expected_Skins, na.rm = TRUE),
