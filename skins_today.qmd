...
if (nrow(nba_today) > 0 && nrow(todays_picks) > 0) {
  ...
  live_data_final <- bind_rows(live_data_team1, live_data_team2) %>%
    mutate(
      Team = trimws(Team),
      live_win_prob = convert_prob_to_num(live_win_prob)
    ) %>%
    mutate(
      # ENHANCED: More robust team name matching with multiple patterns
      Team = case_when(
    ...
    group_by(Team) %>%
    summarise(live_win_prob = first(na.omit(live_win_prob)), .groups = 'drop')
...
if (exists("todays_results") && nrow(todays_results) > 0) {
  ...
  final_player_summary <- todays_results_with_cumulative %>%
    ...
    mutate(
      ...
      # Apply color coding to Live Skins Luck column
...
cat("\n\n### Live Data Matching Diagnostics\n\n")

# Diagnostic 1: Teams in todays_picks without Live_Expected_Skins
if (exists("todays_picks") && nrow(todays_picks) > 0) {
  teams_missing_live <- todays_picks %>%
    filter(is.na(Live_Expected_Skins)) %>%
    select(Team, Abbr, Player, Pick, `Expected Skins`)
  
  if (nrow(teams_missing_live) > 0) {
    cat("**Issue #1: Teams Missing Live Expected Skins**\n\n")
    cat("These teams from today's picks did NOT get matched with live data from Gambletron:\n\n")
    print(kable(teams_missing_live, format = "html") %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE))
    cat("\n\n")
  } else {
    cat("✓ **Issue #1 Status: RESOLVED** - All teams have live expected skins values\n\n")
  }
}

# Diagnostic 2: Teams from Gambletron that weren't standardized
if (exists("nba_today") && exists("live_data_final")) {
  original_teams <- unique(c(nba_today$team_1, nba_today$team_2))
  standardized_teams <- unique(live_data_final$Team)
  
  # Check if any original teams don't appear in standardized list
  # (this would indicate a standardization failure)
  cat("**Issue #2: Gambletron Team Names**\n\n")
  cat(paste0("- Original team names from Gambletron: ", length(original_teams), "\n"))
  cat(paste0("- Successfully standardized: ", length(standardized_teams), "\n\n"))
  
  if (length(original_teams) > length(standardized_teams)) {
    cat("⚠ **Warning:** Some Gambletron team names may have been duplicates or failed to standardize\n\n")
  } else {
    cat("✓ All Gambletron team names were processed\n\n")
  }
}

# Diagnostic 3: Summary of Live_Expected_Skins in matchup table
if (exists("matchup_prep")) {
  live_summary <- matchup_prep %>%
    summarise(
      Total_Games = n(),
      Games_With_Live_Data = sum(!is.na(Live_Expected_Skins)),
      Games_Missing_Live_Data = sum(is.na(Live_Expected_Skins)),
      Pct_With_Live_Data = mean(!is.na(Live_Expected_Skins)) * 100
    )
  
  cat("**Issue #3: Live Data Coverage in Matchup Table**\n\n")
  cat(paste0("- Total games today: ", live_summary$Total_Games, "\n"))
  cat(paste0("- Games with live data: ", live_summary$Games_With_Live_Data, "\n"))
  cat(paste0("- Games missing live data: ", live_summary$Games_Missing_Live_Data, "\n"))
  cat(paste0("- Coverage percentage: ", sprintf("%.1f", live_summary$Pct_With_Live_Data), "%\n\n"))
  
  if (live_summary$Pct_With_Live_Data == 100) {
    cat("✓ **Issue #3 Status: RESOLVED** - All games have live data\n\n")
  } else {
    cat("⚠ **Issue #3 Status: PARTIAL** - Some games still missing live data\n\n")
  }
}

# Diagnostic 4: Check for distinct() issues
if (exists("live_data_final")) {
  cat("**Issue #4: Duplicate Removal Check**\n\n")
  cat(paste0("- Unique teams in live_data_final: ", nrow(live_data_final), "\n"))
  cat(paste0("- Expected NBA teams: 30\n\n"))
  
  if (nrow(live_data_final) > 30) {
    cat("⚠ **Warning:** More than 30 teams found - possible duplicates remaining\n\n")
  } else if (nrow(live_data_final) < 30) {
    cat("⚠ **Warning:** Fewer than 30 teams found - some games may not be scraped yet\n\n")
  } else {
    cat("✓ Correct number of teams found\n\n")
  }
}

cat("\n---\n\n")
cat("*Note: Missing live data for games that haven't started yet is expected behavior.*\n")
```
