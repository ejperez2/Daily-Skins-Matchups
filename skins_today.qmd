...
draft_data <- tribble(
  ...
  "Utah Jazz", "Jazz|Utah", "UTA", "L", "<u>Eristeo</u>",
  ...
  "Phoenix Suns", "Suns|Phoenix", "PHX", "L", "<u>Eristeo</u>",
  ...
  "Los Angeles Clippers", "Clippers|LA Clippers", "LAC", "W", "<u>Eristeo</u>",
  ...
  "Indiana Pacers", "Pacers|Indiana", "IND", "L", "<u>Eristeo</u>",
  ...
  "San Antonio Spurs", "Spurs|San Antonio", "SAS", "W", "<u>Eristeo</u>",
...
if (use_csv_for_sportsline) {
  ...
  cat("NOTICE: In blackout window (11pm-2am CT). Loading data from CSV.\n")
  ...
  if (file.exists(file_path)) {
    ...
    if ("game_id" %in% colnames(csv_data)) {
      ...
      todays_picks <- csv_data %>%
        mutate(
          ...
          Live_Expected_Skins = NA_real_  # No live data during blackout
        ) %>%
    ...
    # Blackout window, but no file found - try to scrape anyway
    cat("WARNING: In blackout window, but no CSV found. Attempting to scrape sportsline...\n")
    use_csv_for_sportsline <- FALSE
  }
}
  ...
  todays_picks <- todays_picks %>%
    ...
    mutate(
      ...
      Live_Expected_Skins = case_when(
        is.na(live_win_prob) ~ Live_Expected_Skins,  # Keep existing value if no new data
        Pick == "W" ~ live_win_prob,
        Pick == "L" ~ 1 - live_win_prob,
        TRUE ~ Live_Expected_Skins
      )
    ) %>%
    select(-live_win_prob)  # Remove the intermediate column
}
...
if (nrow(todays_picks) > 0) {
  ...
  matchup_prep <- todays_picks %>%
    mutate(
      # Calculate cumulative Live Expected Skins (completed games use Actual_Skins)
  ...
  matchup_table <- matchup_prep %>%
    ...
    summarise(
      ...
      Live_Skin_Prob_1 = first(Cumulative_Live_Expected), 
      ...
      Live_Skin_Prob_2 = if(n() > 1) nth(Cumulative_Live_Expected, 2) else NA,
    ...
    mutate(
      ...
      status_order = case_when(
        ...
        str_detect(Status, "Q2") ~ 4,
        str_detect(Status, "Q1") ~ 5,
        TRUE ~ 6  # Not Started and other
      )
    ) %>%
    ...
    mutate(
      ...
      Player_1_Display = case_when(
        Status == "Final" & !is.na(Won_Skin_1) & Won_Skin_1 ~ 
          cell_spec(Player_1, background = "#90EE90", color = "#000000 !important", bold = TRUE, format = "html", extra_css = "color: #000000 !important;", escape = FALSE),
        Status == "Final" & !is.na(Won_Skin_1) & !Won_Skin_1 ~ 
          cell_spec(Player_1, background = "#FFB6C6", color = "#000000 !important", bold = TRUE, format = "html", extra_css = "color: #000000 !important;", escape = FALSE),
        TRUE ~ Player_1
      ),
      Player_2_Display = case_when(
        is.na(Player_2) ~ "",
        Status == "Final" & !is.na(Won_Skin_2) & Won_Skin_2 == TRUE ~ 
          cell_spec(Player_2, background = "#90EE90", color = "#000000 !important", bold = TRUE, format = "html", extra_css = "color: #000000 !important;", escape = FALSE),
        Status == "Final" & !is.na(Won_Skin_2) & Won_Skin_2 == FALSE ~ 
          cell_spec(Player_2, background = "#FFB6C6", color = "#000000 !important", bold = TRUE, format = "html", extra_css = "color: #000000 !important;", escape = FALSE),
        TRUE ~ Player_2
      ),
        ...
        Player_1_Display = case_when(
          Status == "Final" & !is.na(Won_Skin_1) & Won_Skin_1 ~ 
            cell_spec(Player_1, background = "#90EE90", color = "#000000 !important", 
                      bold = TRUE, format = "html", extra_css = "color: #000000 !important;", escape = FALSE),
          Status == "Final" & !is.na(Won_Skin_1) & !Won_Skin_1 ~ 
...
if (exists("todays_results") && nrow(todays_results) > 0) {
  ...
  total_row <- data.frame(
    ...
    `Live Skins Luck` = sum(todays_results_with_cumulative$Cumulative_Live_Expected, na.rm = TRUE) - 
