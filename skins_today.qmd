...
match_team <- function(matchup_str, draft_data) {
  ...
  for (i in 1:nrow(draft_data)) {
    # Split the Matchup_Key by pipe to get alternatives
    ...
    for (key in keys_upper) {
      if (str_detect(matchup_upper, fixed(key))) {
        return(draft_data$Matchup_Key[i])
      }
    }
    
    # Also check full team name
    if (str_detect(matchup_upper, fixed(team_upper))) {
      return(draft_data$Matchup_Key[i])
    }
  }
...
todays_picks <- game_data_cleaned %>%
  ...
  filter(str_detect(consensus, "(Current|Open):")) %>%
  mutate(
    ...
    game_time_utc = parse_date_time(
      str_remove(game_time_str_with_year, " UTC"),
  ...
  mutate(
    # Prefer Current odds over Open odds
    current_odds_str = str_extract(consensus, "Current:\\s*[+-]?\\d+"),
    open_odds_str = str_extract(consensus, "Open:\\s*[+-]?\\d+"),
    odds_to_use = if_else(!is.na(current_odds_str), current_odds_str, open_odds_str),
    moneyline = as.numeric(str_extract(odds_to_use, "[+-]?\\d+"))
  ) %>%
