  ...
  matchup_prep <- todays_picks %>%
    ...
    mutate(
      ...
      Won_Skin = case_when(
        ...
        Won_Game == FALSE & Pick == "L"  "Detroit Pistons", "Pistons|Detroit", "DET", "W", "Brian",
  "Indiana Pacers", "Pacers|Indiana", "IND", "L", "Eristeo",
  "Miami Heat", "Heat|Miami", "MIA", "L", "Matt",
  "Philadelphia 76ers", "76ers|Philadelphia", "PHI", "W", "Matt",
  "Toronto Raptors", "Raptors|Toronto", "TOR", "L", "Brian",
...
if (nrow(nba_today) > 0 && nrow(todays_picks) > 0) {
  ...
  todays_picks <- todays_picks %>%
    ...
    mutate(
      # *** START FIX ***
      # Calculate Live Expected Skins:
      # Use live_win_prob if available.
      # If it's NA (e.g., game hasn't started), 
      # default to the pre-game "Expected Skins".
      Live_Expected_Skins = case_when(
        !is.na(live_win_prob) & Pick == "W" ~ live_win_prob,
        !is.na(live_win_prob) & Pick == "L" ~ 1 - live_win_prob,
        is.na(live_win_prob) ~ `Expected Skins`,
        TRUE ~ `Expected Skins` # Failsafe
      )
      # *** END FIX ***
    )
} else {
  # If scrape fails or no games, create column and set to default
  todays_picks <- todays_picks %>%
...
## Today's Results & Skins Luck (breaks around 10:30p)
...
if (nrow(todays_picks) > 0) {
  ...
  # --- FIX: ALWAYS use the live URL for scores ---
  # The blackout logic in 'expected_skins_data' already handles
  # loading the correct *picks* from the CSV.
  url_scores <- "https://plaintextsports.com/nba/"
  # --- END FIX ---
  ...
  matchup_prep <- todays_picks %>%
    ...
    # Override Live_Expected_Skins if the game is Final
    mutate(
      Live_Expected_Skins = case_when(
        Game_Status == "Final" & !is.na(Won_Skin) & Won_Skin == TRUE ~ 1.0,
        Game_Status == "Final" & !is.na(Won_Skin) & Won_Skin == FALSE ~ 0.0,
        TRUE ~ Live_Expected_Skins # Keep original value if not final
      )
    ) %>%
...
if (nrow(todays_picks) > 0) {
  ...
  # This joins the scraped 'final_team_results' with 'todays_picks'
  # (which may have come from the CSV)
  ...
  # This will overwrite the CSV loaded during blackout with FINAL results
  ...
  completed_games_table <- todays_results %>%
    ...
    arrange(Player, Team) %>%
    mutate(`Won Game?` = as.character(`Won Game?`)) # Keep blank row fix
  
  if (nrow(completed_games_table) > 0) {
    
    # Create total row (with blank row fix)
    total_row_completed <- data.frame(
      Player = "TOTAL",
      Team = "",  
      Pick = "",  
      `Won Game?` = "", 
    ...
    completed_games_table_with_total <- bind_rows(completed_games_table, total_row_completed)
    
    
    knitr::kable(
      completed_games_table_with_total, 
      digits = 1, 
      align = 'c', 
      caption = "Skins Results for Completed Games",
      na = "" 
    ) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
      column_spec(c(5, 7), bold = TRUE) %>%
      row_spec(nrow(completed_games_table_with_total), bold = TRUE, background = "#444444")
...
if (exists("todays_results") && nrow(todays_results) > 0) {
  ...
  # Bind total row to the table
  final_summary_with_total <- bind_rows(final_player_summary, total_row_final)
  # --- *** NEW CODE END *** ---
  
  knitr::kable(
    final_summary_with_total,
    digits = 1,
    align = 'c',
    ...
    row_spec(nrow(final_summary_with_total), bold = TRUE, background = "#444444") # Add bold row
}
```
  ...
  if (nrow(yesterday_data) > 0) {
    ...
    if (length(yesterday_boxes) > 0) {
      ...
      if (nrow(yesterday_matchup_table) > 0) {
        ...
        # --- *** NEW CODE START *** ---
        # Create total row
        total_row_yesterday <- data.frame(
          Player = "TOTAL",
        ...
        # Bind total row
        yesterday_player_summary_with_total <- bind_rows(yesterday_player_summary, total_row_yesterday)
        # --- *** NEW CODE END *** ---
        
        cat("<br>")
        print(knitr::kable(
          yesterday_player_summary_with_total, # Use new data frame
          digits = 1,
          align = 'c',
          ...
          row_spec(nrow(yesterday_player_summary_with_total), bold = TRUE, background = "#444444")) # Add row spec
      }
    }
  }
} else {
...
# UPDATED: Nov 4 baseline data
ytd_baseline <- tribble(
  ~Player, ~Games, ~Total_Expected_Skins, ~Total_Actual_Skins, ~Cumulative_Skins_Luck,
  "Adam",    37, 21.4, 23,  1.6,
  "Eristeo", 33, 23.1, 22, -1.1,
  "Kenneth", 35, 22.0, 17, -5.0,
  "Brian",   35, 22.0, 24,  2.0,
  "Thomas",  33, 20.7, 16, -4.7,
  "Matt",    33, 23.3, 25,  1.7
)
...
# Set it to Nov 4 so that only Nov 4+ data gets added on top of the baseline
baseline_date_file <- file.path(history_dir, "baseline_date.txt")
if (!file.exists(baseline_date_file)) {
  # Set baseline date to Nov 4 - baseline includes everything through Nov 3
  writeLines("2025-11-04", baseline_date_file)
}
